...
const List = () => {
    ...
    const { data: products, isLoading, isSuccess, isError, error } = useGetProductsQuery(input)


    let content;
    const productsAll = products?.data;
    ...
    function Status(status) {
        if (status == "publish") {
            return <div className='w-16 h-6 flex items-center justify-center bg-[#45edbd18] rounded-md'>
                <p className='text-[#35BF97] text-xs m-0 p-0'>{status}</p>
            </div>
        }
        else if (status == "draft") {
            return <div className='w-16 h-6 flex items-center justify-center bg-[#ffbb0e1b] rounded-md'>
                            ...
                            <table className="w-full text-xs text-left  ">
                                ...
                                <tbody>
                                    {productsAll.map(product => (
                                        <tr key={product.id} className="bg-white border-b text-black font-medium ">
                                            <td className="p-4 font-medium  flex items-center gap-x-2">
                                                <img src={product.productImage} className="w-10 h-10 object-cover rounded-sm" alt="" />
                                                <p>{product.name}</p>
                                            </td>
                                            <td className="p-4">
                                                {product.quantity}
